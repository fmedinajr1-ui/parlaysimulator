import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.38.4";

// ─────────────────────────────────────────────────────────────────────────────
// high-conviction-analyzer  (FIXED)
//
// BUG 6 — bot_daily_parlays legs were fed back as a 'bot_parlay' engine source.
//          This created a circular loop: the bot's own picks inflated conviction
//          scores for the next generation cycle. Removed entirely as an engine.
//
// BUG 7 — sideAgreement required ALL engines to agree (.every), so it almost
//          never fired with 5+ engines. Changed to majority threshold: ≥60% of
//          engines must agree for sideAgreement = true.
//
// BUG 8 — engineCountBonus was additive (matches.length * 2, uncapped), which
//          let volume overwhelm edge quality. A weak MEDIUM pick from 6 engines
//          outscored a strong ELITE pick from 2 engines. Changed to a capped
//          multiplier: engine count now scales the edgeScore rather than
//          being added to it as a flat bonus.
//
// BUG 9 — delete-before-insert on high_conviction_results was non-atomic.
//          If the insert batch failed mid-way, today's table was partially empty
//          and parlay generation would read incomplete data. Changed to upsert
//          with conflict resolution on (analysis_date, player_name, prop_type).
// ─────────────────────────────────────────────────────────────────────────────

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
};

function getEasternDate(): string {
  return new Intl.DateTimeFormat("en-CA", {
    timeZone: "America/New_York",
    year: "numeric", month: "2-digit", day: "2-digit",
  }).format(new Date());
}

function normalizePropType(raw: string): string {
  const s = raw.replace(/^(player_|batter_|pitcher_)/, "").toLowerCase().trim();
  if (/points.*rebounds.*assists|pts.*rebs.*asts|^pra$/.test(s)) return "pra";
  if (/points.*rebounds|pts.*rebs|^pr$/.test(s)) return "pr";
  if (/points.*assists|pts.*asts|^pa$/.test(s)) return "pa";
  if (/rebounds.*assists|rebs.*asts|^ra$/.test(s)) return "ra";
  if (/three_pointers|threes_made|^threes$/.test(s)) return "threes";
  return s;
}

interface EnginePick {
  player_name: string;
  prop_type: string;
  side: string;
  confidence?: number;
  engine: string;
}

serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
  const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
  const supabase = createClient(supabaseUrl, supabaseKey);
  const today = getEasternDate();

  try {
    console.log(`[HighConviction] Starting analysis for ${today}`);

    // BUG 6 FIX: removed bot_daily_parlays from the fetch list —
    // bot's own picks are not an independent signal source.
    const [mispricedResult, riskResult, propV2Result, sharpResult, heatResult, mlbCrossRefResult] = await Promise.all([
      supabase.from("mispriced_lines")
        .select("player_name, prop_type, signal, edge_pct, confidence_tier, book_line, player_avg_l10, sport")
        .eq("analysis_date", today),
      supabase.from("nba_risk_engine_picks")
        .select("player_name, prop_type, line, side, confidence_score")
        .eq("game_date", today),
      supabase.from("prop_engine_v2_picks")
        .select("player_name, prop_type, line, side, ses_score, decision")
        .eq("game_date", today),
      supabase.from("sharp_ai_parlays")
        .select("leg_1, leg_2, parlay_type")
        .eq("parlay_date", today),
      supabase.from("heat_parlays")
        .select("legs, parlay_type")
        .eq("parlay_date", today),
      supabase.from("mlb_engine_picks")
        .select("player_name, prop_type, line, side, confidence_score")
        .eq("game_date", today),
    ]);

    const mispricedLines = mispricedResult.data || [];
    if (riskResult.error)     console.error("[HighConviction] Risk query error:", riskResult.error);
    if (mispricedResult.error) console.error("[HighConviction] Mispriced query error:", mispricedResult.error);

    console.log(`[HighConviction] Date: ${today}, Mispriced: ${mispricedLines.length}, Risk: ${riskResult.data?.length||0}, PropV2: ${propV2Result.data?.length||0}, Sharp: ${sharpResult.data?.length||0}, Heat: ${heatResult.data?.length||0}, MLB-CrossRef: ${mlbCrossRefResult.data?.length||0}`);

    // Build engine picks map — 5 independent engines (bot_parlay removed, BUG 6)
    const engineMap = new Map<string, EnginePick[]>();
    const addPick = (pick: EnginePick) => {
      if (!pick.player_name || !pick.prop_type) return;
      const key = `${pick.player_name.toLowerCase()}|${normalizePropType(pick.prop_type)}`;
      if (!engineMap.has(key)) engineMap.set(key, []);
      engineMap.get(key)!.push(pick);
    };

    for (const p of riskResult.data || []) {
      addPick({ player_name: p.player_name, prop_type: p.prop_type, side: p.side||"over", confidence: p.confidence_score, engine: "risk" });
    }
    for (const p of propV2Result.data || []) {
      addPick({ player_name: p.player_name, prop_type: p.prop_type, side: p.side||"over", confidence: p.ses_score, engine: "propv2" });
    }
    for (const parlay of sharpResult.data || []) {
      for (const legKey of ["leg_1", "leg_2"] as const) {
        const leg = parlay[legKey];
        if (leg && typeof leg === "object") {
          addPick({ player_name: leg.player_name||"", prop_type: leg.prop_type||leg.stat_type||"", side: leg.side||"over", engine: "sharp" });
        }
      }
    }
    for (const parlay of heatResult.data || []) {
      if (Array.isArray(parlay.legs)) {
        for (const leg of parlay.legs) {
          if (leg && typeof leg === "object") {
            addPick({ player_name: leg.player_name||"", prop_type: leg.prop_type||leg.stat_type||"", side: leg.side||"over", engine: "heat" });
          }
        }
      }
    }
    for (const p of mlbCrossRefResult.data || []) {
      addPick({ player_name: p.player_name, prop_type: p.prop_type, side: p.side||"over", confidence: p.confidence_score, engine: "mlb_cross_ref" });
    }

    // Cross-reference mispriced lines against engine picks
    const plays: any[] = [];

    for (const ml of mispricedLines) {
      const key = `${ml.player_name.toLowerCase()}|${normalizePropType(ml.prop_type)}`;
      const matches = engineMap.get(key);
      if (!matches || matches.length === 0) continue;

      const mispricedSide = ml.signal.toLowerCase();

      // BUG 7 FIX: majority agreement (≥60%) instead of unanimous (.every)
      const agreementCount = matches.filter(m => m.side.toLowerCase() === mispricedSide).length;
      const agreementRatio = agreementCount / matches.length;
      const sideAgreement = agreementRatio >= 0.6;

      // BUG 8 FIX: engine count is a multiplier on edgeScore, not an additive bonus.
      // This preserves the primacy of actual edge quality.
      // Formula: edgeScore * engineMultiplier + tierBonus + agreementBonus + riskBonus
      // engineMultiplier: 1 engine = 1.0x, 2 = 1.2x, 3 = 1.4x, 4+ = 1.5x (capped)
      const engineMultiplier = Math.min(1.5, 1.0 + (matches.length - 1) * 0.2);

      const edgeScore = Math.min(Math.abs(ml.edge_pct) / 10, 10);
      const scaledEdgeScore = edgeScore * engineMultiplier;

      const tierBonus = ml.confidence_tier === "ELITE" ? 3 : ml.confidence_tier === "HIGH" ? 2 : 1;
      const agreementBonus = sideAgreement ? 3 : 0;

      // Direction bonus: partial credit for engines that agree on direction
      const directionBonus = agreementCount * 1.0; // 1 pt per agreeing engine (was 1.5x all matches)

      const riskConfidence = matches.find(m => m.engine === "risk")?.confidence || 0;
      const riskBonus = riskConfidence > 0 ? riskConfidence / 20 : 0;

      const convictionScore = scaledEdgeScore + tierBonus + agreementBonus + directionBonus + riskBonus;

      plays.push({
        player_name: ml.player_name,
        prop_type: normalizePropType(ml.prop_type),
        displayPropType: ml.prop_type,
        signal: ml.signal,
        edge_pct: ml.edge_pct,
        confidence_tier: ml.confidence_tier,
        current_line: ml.book_line,
        player_avg: ml.player_avg_l10,
        sport: ml.sport,
        engines: matches.map(m => ({ engine: m.engine, side: m.side, confidence: m.confidence })),
        engine_count: matches.length,
        agreement_ratio: Math.round(agreementRatio * 100),
        sideAgreement,
        convictionScore,
      });
    }

    plays.sort((a, b) => b.convictionScore - a.convictionScore);

    const engineCounts: Record<string, number> = {};
    for (const p of plays) {
      for (const e of p.engines) {
        engineCounts[e.engine] = (engineCounts[e.engine] || 0) + 1;
      }
    }

    const stats = {
      total: plays.length,
      allAgree: plays.filter(p => p.sideAgreement).length,
      engineCounts,
    };

    console.log(`[HighConviction] Found ${plays.length} overlaps, ${stats.allAgree} with majority agreement`);

    // BUG 9 FIX: upsert instead of delete-then-insert to avoid partial-write window
    if (plays.length > 0) {
      const rows = plays.map(p => ({
        analysis_date: today,
        player_name: p.player_name,
        prop_type: p.prop_type,
        display_prop_type: p.displayPropType,
        signal: p.signal,
        edge_pct: p.edge_pct,
        confidence_tier: p.confidence_tier,
        current_line: p.current_line,
        player_avg: p.player_avg,
        sport: p.sport,
        engines: p.engines,
        engine_count: p.engine_count,
        agreement_ratio: p.agreement_ratio,
        side_agreement: p.sideAgreement,
        conviction_score: p.convictionScore,
        updated_at: new Date().toISOString(),
      }));

      const chunkSize = 50;
      let upserted = 0;
      for (let i = 0; i < rows.length; i += chunkSize) {
        const { error: upsertError } = await supabase
          .from("high_conviction_results")
          .upsert(rows.slice(i, i + chunkSize), {
            onConflict: "analysis_date,player_name,prop_type",
            ignoreDuplicates: false,
          });
        if (upsertError) {
          console.error("[HighConviction] Upsert error:", upsertError);
        } else {
          upserted += Math.min(chunkSize, rows.length - i);
        }
      }

      // Clean up any stale rows from a previous wider run (e.g. more players yesterday)
      // Only delete rows NOT in today's play list
      const keepNames = new Set(plays.map(p => p.player_name));
      const { data: staleRows } = await supabase
        .from("high_conviction_results")
        .select("id, player_name")
        .eq("analysis_date", today);
      const staleIds = (staleRows || [])
        .filter(r => !keepNames.has(r.player_name))
        .map(r => r.id);
      if (staleIds.length > 0) {
        await supabase.from("high_conviction_results").delete().in("id", staleIds);
        console.log(`[HighConviction] Cleaned ${staleIds.length} stale rows`);
      }

      console.log(`[HighConviction] Upserted ${upserted} results`);
    }

    // Telegram — top 15
    const top15 = plays.slice(0, 15);
    try {
      const teleResp = await fetch(`${supabaseUrl}/functions/v1/bot-send-telegram`, {
        method: "POST",
        headers: { Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ type: "high_conviction_report", data: { plays: top15, stats } }),
      });
      const teleResult = await teleResp.json();
      console.log("[HighConviction] Telegram sent:", teleResult.success);
    } catch (teleErr) {
      console.error("[HighConviction] Telegram failed:", teleErr);
    }

    return new Response(JSON.stringify({
      success: true,
      total_overlaps: plays.length,
      side_agreement: stats.allAgree,
      engine_counts: engineCounts,
      top_plays: top15,
    }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });

  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HighConviction] Error:", msg);
    return new Response(JSON.stringify({ success: false, error: msg }), {
      status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
});
