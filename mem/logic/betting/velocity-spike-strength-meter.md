...
- **Always-on grading (`velocity-spike-settler` edge function + cron every 15 min)**: grades EVERY velocity_spike alert against `*_player_game_logs` regardless of broadcast gate, including `NEUTRAL`, `LEAN`, `WEAK`, `STRONG` and missing verdicts. Mirrors into `fanduel_prediction_accuracy` with `signal_factors.engine_verdict` preserved so accuracy-by-verdict queries work. Uses `settlement_method='velocity_spike_game_log_v1'`. Dedupes by `(event_id,player_name,prop_type,signal_type)` per upsert chunk to avoid ON CONFLICT collisions.
