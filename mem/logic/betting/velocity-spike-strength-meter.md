...
- Pre-broadcast **engine-reasoning gate**: velocity_spike alerts are hard-skipped when `metadata.engine_reasoning.verdict` is missing, `NEUTRAL`, or `WEAK`. Only `STRONG` and `LEAN` per-player reasoning is allowed to broadcast — prevents "BACK X" recs with `🟡 NEUTRAL · juice +X · volatile minutes` underneath.
