...
function RoleBadge({ role }: { role: string }) {
  const roleConfig: Record<string, { icon: any; color: string; label: string }> = {
    'STAR': { icon: Star, color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', label: 'Star' },
    'SECONDARY_GUARD': { icon: Users, color: 'bg-blue-500/20 text-blue-400 border-blue-500/30', label: 'Guard' },
    'WING': { icon: Crosshair, color: 'bg-purple-500/20 text-purple-400 border-purple-500/30', label: 'Wing' },
    'BIG': { icon: Shield, color: 'bg-orange-500/20 text-orange-400 border-orange-500/30', label: 'Big' },
  };
  ...
  return (
    <div className="p-4 bg-card/50 rounded-lg border border-border/50 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        <RoleBadge role={pick.player_role} />
