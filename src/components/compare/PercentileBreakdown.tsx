  ...
  return (
    <FeedCard className="p-4 mt-4">
      ...
      <p className="text-xs text-muted-foreground mb-4">
        Shows the range of possible outcomes. P5 = worst 5% of cases, P95 = best 5% of cases.
      </p>
          ...
          return (
            <div key={idx} className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Parlay {idx + 1}</span>
              ...
              <div className="relative h-10 bg-muted/30 rounded-lg overflow-hidden">
                ...
                {/* Box (P25 to P75) */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 h-6 rounded bg-primary/20 border border-primary/40"
                  style={{
                ...
                {/* P5 marker */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 w-2 h-6 rounded-sm bg-destructive/60"
                  style={{ left: `${getPosition(percentiles.p5)}%`, transform: 'translate(-50%, -50%)' }}
                />

                {/* P25 marker */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 w-1 h-6 bg-neon-orange/60"
                  style={{ left: `${getPosition(percentiles.p25)}%`, transform: 'translate(-50%, -50%)' }}
                />

                {/* P50 (Median) marker */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 w-1.5 h-8 bg-primary rounded-full"
                  style={{ left: `${getPosition(percentiles.p50)}%`, transform: 'translate(-50%, -50%)' }}
                />

                {/* P75 marker */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 w-1 h-6 bg-neon-green/60"
                  style={{ left: `${getPosition(percentiles.p75)}%`, transform: 'translate(-50%, -50%)' }}
                />

                {/* P95 marker */}
                <div 
                  className="absolute top-1/2 -translate-y-1/2 w-2 h-6 rounded-sm bg-neon-green"
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">
                  95% of the time you'll lose at least {formatValue(-percentiles.p5 > 0 ? percentiles.p5 : percentiles.p5)}
                </span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <TrendingUp className="w-3 h-3 text-neon-green" />
                <span className="text-muted-foreground">
                  5% chance to win {formatValue(percentiles.p95)}
                </span>
              </div>
            </div>
          );
      ...
      {results.length > 1 && (
        <div className="mt-4 pt-4 border-t border-border">
          <h4 className="text-sm font-medium mb-2">Risk Comparison</h4>
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <span className="text-muted-foreground">Lowest Downside Risk:</span>
