...
const HARD_CONFLICTS = [
  ...
  // Two high-usage players same team, both OVER points (usage competition)
  {
    ...
    check: (a: SweetSpotPick, b: SweetSpotPick) =>
      ...
      a.side?.toLowerCase() === "over" &&
      b.side?.toLowerCase() === "over" &&
      (a.line || 0) >= 15 &&
      (b.line || 0) >= 15,
  },
];
    ...
    for (let j = i + 1; j < legs.length; j++) {
      for (const conflict of HARD_CONFLICTS) {
        if (conflict.check(legs[i], legs[j])) {
          console.log(
            `[Synergy] HARD CONFLICT: ${conflict.rule} between ${legs[i].player_name} and ${legs[j].player_name}`,
          );
          return true;
        }
      }
...
export function calculateLegSynergy(
  ...
  const sameTeam = leg1.team_name && leg2.team_name && leg1.team_name.toLowerCase() === leg2.team_name.toLowerCase();

  if (sameGame || sameTeam) {
    // HARD CONFLICT: Same player, opposite sides
    if (
      leg1.player_name?.toLowerCase() === leg2.player_name?.toLowerCase() &&
    ...
    if (
      sameTeam &&
      leg1.side?.toLowerCase() === "over" &&
      leg2.side?.toLowerCase() === "over" &&
    ...
    if (vegasTotal < SYNERGY_RULES.SLOW_GAME.totalThreshold) {
      if (
        normalizeProp(leg1.prop_type).includes("rebound") &&
        leg1.side?.toLowerCase() === "over" &&
        normalizeProp(leg2.prop_type).includes("rebound") &&
        leg2.side?.toLowerCase() === "over"
      ) {
        synergy += 1;
    ...
    if (vegasTotal > SYNERGY_RULES.FAST_GAME.totalThreshold) {
      const leg1PtsAst =
        normalizeProp(leg1.prop_type).includes("point") || normalizeProp(leg1.prop_type).includes("assist");
      const leg2PtsAst =
        normalizeProp(leg2.prop_type).includes("point") || normalizeProp(leg2.prop_type).includes("assist");
...
function passesMinEdgeThreshold(pick: SweetSpotPick): boolean {
  ...
  if (edge < threshold) {
    console.log(
      `[EdgeFilter] ${pick.player_name} ${pick.prop_type} ${pick.side}: Edge ${edge.toFixed(2)} < threshold ${threshold} - BLOCKED`,
    );
    return false;
  }

  console.log(
    `[EdgeFilter] ${pick.player_name} ${pick.prop_type} ${pick.side}: Edge ${edge.toFixed(2)} >= ${threshold} ✓ PASS`,
  );
  return true;
}

// Safe prop normalization helper - handles null/undefined and strips non-alpha chars
const normalizeProp = (p?: string | null): string => (p || "").toLowerCase().replace(/[^a-z]/g, ""); // "Points + Rebounds" -> "pointsrebounds"
...
const WINNING_PATTERN_RULES: Record<
  string,
  {
    minLine?: number;
    maxLine?: number;
...
export function buildSweetSpotParlayCore(input: BuilderInput): BuilderOutput {
  ...
  const getOpponentDefenseRank = (pick: SweetSpotPick): number | undefined => {
    ...
    if (!allowedStatTypes.has(statType)) {
      const propNorm = normalizeProp(pick.prop_type);
      statType = propNorm.includes("rebound") ? "rebounds" : propNorm.includes("assist") ? "assists" : "points";
    }
...
export function useSweetSpotParlayBuilder() {
  ...
  const {
    ...
    queryFn: async (): Promise<QueryResult> => {
      ...
      let targetPlayers = new Set(todayActiveProps.map((p) => p.player_name?.toLowerCase()).filter(Boolean));
      ...
      const { data: matchupHistoryData } = await supabase
        ...
        .select(
          "player_name, opponent, prop_type, games_played, avg_stat, hit_rate_over, hit_rate_under, max_stat, min_stat",
        );
      ...
      const blockedSet = new Set(
        (blockedPicks || []).map(
          (p) => `${p.player_name?.toLowerCase()}_${p.prop_type?.toLowerCase()}_${p.side?.toLowerCase()}`,
        ),
      );
      ...
      console.log(
        `🎯 Game Environment Validation: ${validationCounts.approved} 🟢 | ${validationCounts.conditional} 🟡 | ${validationCounts.rejected} 🔴`,
      );

      // Get player team data from cache
      const { data: playerCache } = await supabase.from("bdl_player_cache").select("player_name, team_name");
      ...
      const reliabilityMap = new Map<
        string,
        { tier: string; hitRate: number; modifier: number; shouldBlock: boolean }
      >();
      ...
      const { data: categoryPicks, error: categoryError } = await supabase
        ...
        .order("player_name", { ascending: true }) // CRITICAL FIX: Secondary sort for determinism
        .order("prop_type", { ascending: true }); // CRITICAL FIX: Tertiary sort for complete determinism
      ...
      const validCategoryPicks = (categoryPicks || []).filter((pick) => {
        ...
        if (
          ...
          console.log(
            `[SweetSpotParlay] Blocking archetype-misaligned category: ${pick.player_name} (${pick.archetype}) for ${pick.prop_type}`,
          );
          return false;
        }
        ...
        if (blockedSet.has(blockKey)) {
          console.log(
            `[SweetSpotParlay] Blocking matchup-blocked category: ${pick.player_name} ${pick.prop_type} ${pick.recommended_side}`,
          );
          return false;
        }
      ...
      console.log(
        `[SweetSpotParlay] Proven category picks: ${validCategoryPicks.length} (from ${categoryPicks?.length || 0})`,
      );
      ...
      const validRiskPicks = (riskPicks || []).filter((pick) => {
        ...
        if (
          ...
          console.log(
            `[SweetSpotParlay] Blocking archetype-misaligned risk: ${pick.player_name} (${pick.archetype}) for ${pick.prop_type}`,
          );
          return false;
        }
        ...
        if (blockedSet.has(blockKey)) {
          console.log(
            `[SweetSpotParlay] Blocking matchup-blocked risk: ${pick.player_name} ${pick.prop_type} ${pick.side}`,
          );
          return false;
        }
        ...
        if (categoryRec && categoryRec.side !== pick.side?.toLowerCase()) {
          console.log(
            `[SweetSpotParlay] Skipping risk pick - category recommends ${categoryRec.side}, risk says ${pick.side}: ${pick.player_name}`,
          );
          return false;
        }
      ...
      console.log(
        `[SweetSpotParlay] Risk engine picks: ${validRiskPicks.length} (filtered from ${riskPicks?.length || 0})`,
      );
      ...
      const validatedPicks = allPicks.filter((pick) => {
        ...
        if (validation.status === "REJECTED") {
          console.log(
            `[GameEnvValidator] ❌ REJECTED: ${pick.player_name} ${pick.prop_type} ${pick.side} - ${validation.reason}`,
          );
        ...
        if (validation.status === "CONDITIONAL") {
          if ((pick.l10HitRate || 0) >= 0.7) {
            console.log(
              `[GameEnvValidator] 🟡 CONDITIONAL (allowed): ${pick.player_name} - high L10 hit rate (${((pick.l10HitRate || 0) * 100).toFixed(0)}%) overrides`,
            );
      ...
      console.log(
        `[SweetSpotParlay] After Game Environment Validation: ${validatedPicks.length}/${allPicks.length} picks passed`,
      );
      console.groupEnd();
  ...
  const slateStatus = queryResult?.slateStatus || {
    currentDate: getEasternDate(),
    displayedDate: getEasternDate(),
    isNextSlate: false,
  };
